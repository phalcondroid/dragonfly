...
class DragonflyContainerHelper {
  /// passed getIt instance
  final DragonflyContainer getIt;
  ...
  DragonflyContainerHelper(this.getIt,
  ...
  /// asynchronously inside of [DragonflyContainer]
  Future<DragonflyContainer> initScopeAsync(String name,
      {required Future<void> Function(DragonflyContainerHelper gh) init,
      ScopeDisposeFunc? dispose}) {
    final Completer<DragonflyContainer> completer =
        Completer<DragonflyContainer>();
    getIt.pushNewScope(
      scopeName: name,
      init: (getIt) async {
        await init(this);
  ...
  /// a helper method to push a new scope and init it's dependencies
  /// inside of [DragonflyContainer]
  DragonflyContainer initScope(String name,
      {required void Function(DragonflyContainerHelper gh) init,
      ScopeDisposeFunc? dispose}) {
    getIt.pushNewScope(
      scopeName: name,
      init: (_) => init(this),
