...
///
/// This adapter wraps the standard HTTP adapter and adds the session
/// token to all requests when a session is active.
///
/// Example:
/// ```dart
/// // In DragonflyApp configuration
/// DragonflyApp(
///   networkConfig: DragonflyNetworkConfig(baseUrl: 'https://api.example.com'),
///   useAuthenticatedAdapter: true, // Uses this adapter instead of basic HTTP adapter
/// ).init();
/// ```
class AuthenticatedNetworkAdapter implements DragonflyBaseNetworkAdapter {
  ...
  /// Merge headers from config, session, and options
  Map<String, String> _buildHeaders(DragonflyNetworkOptions? options) {
    ...
    if (_session.isAuthenticated && _session.token != null) {
      ...
      headers[headerName] = '$tokenType ${_session.token}';
    }
  ...
  Future<void> _handleUnauthorized() async {
    _log.warning('Received 401 Unauthorized', tag: 'AuthNetwork');
  ...
  Future<http.Response> _callHttpMethod(
    ...
    if (enableLogging) {
      _log.request(
        ...
        source: 'AuthenticatedNetworkAdapter',
      );
    }
  ...
  void _validateResponse(http.Response response) {
    if (response.statusCode >= 400) {
      throw AuthenticatedHttpException(
...
class AuthenticatedHttpException implements Exception {
  final String message;
  final int statusCode;
  final String body;

  const AuthenticatedHttpException(this.message, this.statusCode, this.body);

  bool get isUnauthorized => statusCode == 401;
  bool get isForbidden => statusCode == 403;

  @override
  String toString() => 'AuthenticatedHttpException: $message';
}
